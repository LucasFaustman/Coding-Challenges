...
function allNonConsecutive (arr) {
    //create a res array
     ...
     for (let i = 1; i < arr.length; i++) {
         //if previous iteration +1 is not equal to our curr, push to res array
       if (arr[i-1] +1 !== arr[i]) {
           res.push({
             'i': i,
             'n': arr[i]
   ...
   //Will be an array. will all be numbers. unclear if always ints. numbers will always be unique
   //in asc order. could be positive, negative
   
   //Return an array of objects with 2 properties. one is the index in which the number is not consecutive
   //another property is the non consecutive number itself
