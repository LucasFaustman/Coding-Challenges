...
function minimumSteps(numbers, value){
    ...
    //if our first number is greater than our target, return 0
    if (numbers[0] > value) {
      return 0
    }
    
    //declare index and count
    
    let index = 0
    let count = 0
    
    //while our value is greater than 0
    while (value > 0) {
      //if index is 0, minus first 2 numbers from value, increase index by 2, and count by 1
      if (index === 0) {
    value -= numbers[index] + numbers[index + 1]
      index += 2
      count++
        //if not index 0, decrease value only by iterator, add one to count and index
        } else {
          value -= numbers[index]
          count++
          index++
  ...
  //Will be 2 params. One will be an array of always at least size 3. array will be positive integers
  //might occur more than once. value will always be an integer
  
  //Return how many times we have to add up smallest numbers in array until their sum becomes
